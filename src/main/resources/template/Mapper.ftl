...
<mapper namespace="${namespace}">
  ...
  <!-- 单个插入 -->
  <insert id="insert${entityName}" parameterType="${entityType}" useGeneratedKeys="true"
          keyProperty="id">
  ...
  <update id="delete${entityName}" parameterType="${entityType}">
    update ${tableName} set UPDATED = <#noparse>#{updated,jdbcType=TIMESTAMP}</#noparse>,UPDATEDBY
    = <#noparse>#{updatedby,jdbcType=VARCHAR}</#noparse>, IS_DELETE = 'Y'
    where ID = <#noparse>#{id,jdbcType=BIGINT}</#noparse>
  </update>
  ...
  <update id="deleteBatch" parameterType="java.util.List">
    <foreach collection="list" item="item" index="index" open="" close="" separator=";">
      update ${tableName}
      set UPDATED = <#noparse>#{item.updated,jdbcType=TIMESTAMP}</#noparse>,UPDATEDBY = <#noparse>
      #{item.updatedby,jdbcType=VARCHAR}</#noparse>, IS_DELETE = 'Y'
      where ID = <#noparse>#{item.id,jdbcType=BIGINT}</#noparse>
    </foreach>
  </update>
